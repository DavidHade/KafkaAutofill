...
[Generator]
internal class AvroAutofill : IIncrementalGenerator
{
    private const string AvroAutofillAttribute = "Avro.Autofill.AvroAutofillAttribute";
    private static readonly string AvroGenSrc = LoadResource("Avro.Autofill.AvroSchemaGen.cs");
    private static readonly string AvroIgnoreSrc = LoadResource("Avro.Autofill.AvroIgnoreAttribute.cs");
    private static readonly string AutofillSrc = LoadResource($"{AvroAutofillAttribute}.cs");
        ...
        foreach (var attributeList in classDeclaration.AttributeLists)
        {
            foreach (var attribute in attributeList.Attributes)
            {
                ...
                if (symbol is IMethodSymbol attributeSymbol)
                {
                    ...
                    if (attributeClass.ToDisplayString() == AvroAutofillAttribute)
                    {
        ...
        SourceProductionContext context)
    {
        ...
        var attributeSymbol = compilation.GetTypeByMetadataName(AvroAutofillAttribute);
        ...
        ClassDeclarationSyntax classDeclaration)
    {
        ...
        foreach (var (property, reason) in unsupportedProperties)
        {
            var diagnostic = Diagnostic.Create(
                AvroAutofillAnalyzer.UnsupportedTypeError,
    ...
    private static void RegisterPostInitializationOutput(IncrementalGeneratorPostInitializationContext postContext)
    {
        ...
        postContext.AddSource("AvroAutofillAttribute.g.cs", SourceText.From(AutofillSrc, Encoding.UTF8));
    ...
    {
        var assembly = typeof(AvroAutofill).Assembly;
